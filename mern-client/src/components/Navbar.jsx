...
useEffect(()=>{
  ...
  const navItems = [
    ...
    <header className='w-full bg-transparent fixed top-0 left-0 right-0 transition-all ease-in duration-300'>
        <nav className={`py-4 lg:px-24 ${isSticky ? "sticky top-0 right-0 bg-blue-300" : ""}`}>
            <div className='flex justify-between items-center text-base gap-8'>
                {/* logo */}
                <Link to="/" className='text-2x1 font-blod text-blue-700 flex items-center gap-2' ><FaBlog className='inline-block'/> Books </Link>
